(function () {
  ...
  function showToolbar(toolbarEl) {
    ...
    barEl.onclick = function (e) {
      ...
      if (
        ...
        e.which !== 2 &&
        !e.ctrlKey // not mouse wheel and not ctrl+click
      ) {
        while (target !== this) {
          if (target.href) {
            removeActiveBlocksCls();
  ...
  function renderAjaxRequests() {
    const requestCounter = document.getElementsByClassName(
    ...
    const tbodies = document.getElementsByClassName(
      "yii-debug-toolbar__ajax_requests"
    );
    let state = "ok";
    if (tbodies.length) {
      ...
      if (requestStack.length) {
        const firstItem =
          requestStack.length > 20 ? requestStack.length - 20 : 0;
        for (let i = firstItem; i < requestStack.length; i++) {
          ...
          if (request.statusCode < 300) {
            statusCode.setAttribute(
              "class",
              "yii-debug-toolbar__ajax_request_status yii-debug-toolbar__label_success"
            );
          } else if (request.statusCode < 400) {
            statusCode.setAttribute(
              "class",
              "yii-debug-toolbar__ajax_request_status yii-debug-toolbar__label_warning"
            );
          } else {
            statusCode.setAttribute(
              "class",
              "yii-debug-toolbar__ajax_request_status yii-debug-toolbar__label_error"
            );
          }
        ...
        function () {
          if (self.readyState == 4) {
            stackElement.duration =
              self.getResponseHeader("X-Debug-Duration") ||
              new Date() - stackElement.start;
